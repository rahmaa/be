...
class taqyeem_review_widget extends WP_Widget {
	...
	function taqyeem_review_widget() {
		...
		$this->WP_Widget( 'taqyeem-review-widget','&raquo;  Review Box', $widget_ops, $control_ops );
	}
...
class taqyeem_get_reviews extends WP_Widget {
	...
	function taqyeem_get_reviews() {
		...
		$this->WP_Widget( 'reviews-posts-widget','&raquo;  Reviews Posts', $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		if( empty($thumb) ){ $thumb_size = false; }
			...
			if(empty($thumb_size) || !is_numeric($thumb_size)) $thumb_size = array(380,380);
			else $thumb_size = array($thumb_size,$thumb_size);
		}
...
class taqyeem_types_get_reviews extends WP_Widget {
	...
	function taqyeem_types_get_reviews() {
		...
		$this->WP_Widget( 'reviews-types-widget','&raquo; Reviews Post Types', $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		if( empty($thumb) ){ $thumb_size = false; }
			...
			if(empty($thumb_size) || !is_numeric($thumb_size)) $thumb_size = array(50,50);
			else $thumb_size = array($thumb_size,$thumb_size);
		}
