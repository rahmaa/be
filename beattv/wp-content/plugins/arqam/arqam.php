...
$arq_data = array();
$arq_social_items = array('facebook', 'twitter', 'google+', 'youtube', 'vimeo', 'dribbble', 'github', 'envato', 'soundcloud', 'behance', 'forrst', 'delicious', 'instagram', 'mailchimp', 'foursquare', 'linkedin', 'vk', 'posts', 'comments');
	...
	if( !get_option('arqam_active') ){
		...
		$default_data = array(
			'social' => array(
				...
				'vk'  			=> array(	'id' => 'applevk',	 'text' => __( 'Members' , 'arq' ) ),
	...
	switch ( $arq_item ) {
		...
		if( !empty($arq_options['social']['linkedin']['id']) ){
			...
			if( !empty( $arq_options['social']['linkedin']['type'] ) &&  $arq_options['social']['linkedin']['type'] == 'Group' )
				$linkedin_link = 'groups/'.$arq_options['social']['linkedin']['group'];
			else 
				$linkedin_link = 'company/'.$arq_options['social']['linkedin']['id'];

		?>
			<li class="arq-linkedin"<?php echo $width ?>>
				<a href="http://www.linkedin.com/<?php echo $linkedin_link ?>"<?php echo $new_window ?>>
...
function arq_linkedin_count() {
	...
	elseif( empty($arq_transient['linkedin']) && !empty($arq_data) && !empty( $arq_options['data']['linkedin'] )  ){
		...
		$linkedin_access_token = get_option( 'linkedin_access_token' );

		if( ( !empty( $linkedin_id ) || !empty( $linkedin_group ) ) && !empty( $linkedin_access_token ) ){
			$linkedin_type = $arq_options['social']['linkedin']['type'];
			if( $linkedin_type == 'Group' ){
				$linkedin_group = explode("-", $linkedin_group);
				$linkedin_group_id = end($linkedin_group);
				$resource = "/v1/groups/$linkedin_group_id:(num-members)";
				$params = array('oauth2_access_token' => get_option( 'linkedin_access_token' ), 'format' => 'json' ); 
				$url = 'https://api.linkedin.com' . $resource . '?' . http_build_query($params);
					 
				try {		
					$response = @arq_remote_get( $url, true );
					$result = $response['numMembers'];
				} catch (Exception $e) {
					$result = 0;
				}
			}else{
				$resource = "/v1/companies/$linkedin_id:(num-followers)";
				$params = array('oauth2_access_token' => get_option( 'linkedin_access_token' ), 'format' => 'json' ); 
				$url = 'https://api.linkedin.com' . $resource . '?' . http_build_query($params);
					 
				try {		
					$response = @arq_remote_get( $url, true );
					$result = $response['numFollowers'];
				} catch (Exception $e) {
					$result = 0;
				}
			}
		...
		<p>
			...
			<select class="widefat" id="<?php echo $this->get_field_id( 'layout' ); ?>" name="<?php echo $this->get_field_name( 'layout' ); ?>" >
				<optgroup label="<?php _e( 'Gray Icons' , 'arq' ) ?>">
					<option value="gray_1col" <?php if( $instance['layout'] == 'gray_1col' ) echo "selected=\"selected\""; else echo ""; ?>><?php _e( 'Gray Icons' , 'arq' ) ?> - <?php _e( '1 Column' , 'arq' ) ?></option>
					<option value="gray_2col" <?php if( $instance['layout'] == 'gray_2col' ) echo "selected=\"selected\""; else echo ""; ?>><?php _e( 'Gray Icons' , 'arq' ) ?> - <?php _e( '2 Columns' , 'arq' ) ?></option>
					<option value="gray_3col" <?php if( $instance['layout'] == 'gray_3col' ) echo "selected=\"selected\""; else echo ""; ?>><?php _e( 'Gray Icons' , 'arq' ) ?> - <?php _e( '3 Columns' , 'arq' ) ?></option>
				</optgroup>	
				
				<optgroup label="<?php _e( 'Colored Icons' , 'arq' ) ?>">
					<option value="colored_1col" <?php if( $instance['layout'] == 'colored_1col' ) echo "selected=\"selected\""; else echo ""; ?>><?php _e( 'Colored Icons' , 'arq' ) ?> - <?php _e( '1 Column' , 'arq' ) ?></option>
					<option value="colored_2col" <?php if( $instance['layout'] == 'colored_2col' ) echo "selected=\"selected\""; else echo ""; ?>><?php _e( 'Colored Icons' , 'arq' ) ?> - <?php _e( '2 Columns' , 'arq' ) ?></option>
					<option value="colored_3col" <?php if( $instance['layout'] == 'colored_3col' ) echo "selected=\"selected\""; else echo ""; ?>><?php _e( 'Colored Icons' , 'arq' ) ?> - <?php _e( '3 Columns' , 'arq' ) ?></option>
				</optgroup>	
				
				<optgroup label="<?php _e( 'Flat Icons' , 'arq' ) ?>">
					<option value="flat_2col" <?php if( $instance['layout'] == 'flat_2col' ) echo "selected=\"selected\""; else echo ""; ?>><?php _e( 'Flat Icons' , 'arq' ) ?> - <?php _e( '2 Columns' , 'arq' ) ?></option>
					<option value="flat_3col" <?php if( $instance['layout'] == 'flat_3col' ) echo "selected=\"selected\""; else echo ""; ?>><?php _e( 'Flat Icons' , 'arq' ) ?> - <?php _e( '3 Columns' , 'arq' ) ?></option>
				</optgroup>		
			</select>
		</p>
		<p>
...
function tie_arqam_css() {
	...
	if( !empty( $arq_options['css'] ) ){ ?>
<style type="text/css" media="screen"> 
<?php $css_code =  str_replace("<pre>", "", htmlspecialchars_decode( $arq_options['css'] ) ); 
